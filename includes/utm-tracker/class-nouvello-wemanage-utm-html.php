        ...
        if (isset($meta_array['cookie_consent']['value']) && $meta_array['cookie_consent']['value'] === 'deny') :
           ...
           <tr bgcolor="%4$s">
              <td style="padding:6px 20px">
                ...
                esc_html(__('Cookie Consent', 'nouvello-wp-management')),
                esc_html(strtoupper($meta_array['cookie_consent']['value'])),
                esc_attr('#EAF2FA'),
                esc_attr('#FFFFFF'),
                esc_html(__('User did not consent to cookie tracking.', 'nouvello-wp-management'))
            );
    ...
    public static function get_clid_tag_html($clid)
    {
        ...
        switch ($clid):
            case 'gclid':
                ...
                $html = '<div style="display: inline-block; padding: 2px 10px; border-radius:9999px; background-color: #c53030; color: #ffffff">' . __('Google', 'nouvello-wp-management') . '</div>';
                break;

            case 'fbclid':

                $html = '<div style="display: inline-block; padding: 2px 10px; border-radius:9999px; background-color: #2b6cb0; color: #ffffff">' . __('Facebook', 'nouvello-wp-management') . '</div>';
                break;

            case 'msclkid':

                $html = '<div style="display: inline-block; padding: 2px 10px; border-radius:9999px; background-color: #039be5; color: #ffffff">' . __('Microsoft', 'nouvello-wp-management') . '</div>';
                break;
