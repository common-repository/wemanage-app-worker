...
{
    public const META_PREFIX = '_nouvello_utm_';
    public const DEFAULT_CONVERSION_TYPE = 'order';
    ...
    public static function action_register_conversion_events()
    {
        ...
        Nouvello_WeManage_Utm_Conversion::register_event(array(
          'event' => 'woocommerce',
          'label' => __('WooCommerce', 'nouvello-wp-management'),
    ...
    public static function save_conversion_attribution($attribution, $order)
    {
        ...
        if (!empty($attribution)) :
            if (Nouvello_WeManage_Utm_Settings::get('attribution_format') === 'json') :
                $meta['attribution'] = Nouvello_WeManage_Utm_Functions::wp_json_encode($attribution);
            else :
                $meta = $attribution;
            endif;
        endif;
    ...
    public static function action_process_conversion_on_thankyou()
    {
        ...
        try {
            ...
            if (isset($get_array['key']) && isset($wp->query_vars['order-received'])) :

                //Default Thank you
                $browser_order_id = absint($wp->query_vars['order-received']);
                $browser_key = wc_clean(wp_unslash($get_array['key']));

            elseif (isset($get_array['wcf-key']) && isset($get_array['wcf-order'])) :

                //Cartflow
                $browser_order_id = absint($get_array['wcf-order']);
                $browser_key = wc_clean(wp_unslash($get_array['wcf-key']));
