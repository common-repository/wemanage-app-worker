    ...
    final class Nouvello_WeManage_Worker_Init
    {
        ...
        public function add_connection_key_link($meta, $slug)
        {
            if (WEMANAGE_PLUGIN_FOLDER . '/nouvello-wemanage-worker.php' == $slug) {
                $trigger = '<a href="#/" id="nouvello-worker-connection-key" ns-wmw-key="' . $this->return_activation_key() . '">' . esc_html__('Connection Management', 'nouvello-wp-management') . '</a>';

                $settings = '<a href="' . admin_url('admin.php?page=wemanage_worker_settings') . '">' . esc_html__('Settings', 'nouvello-wp-management') . '</a>';
