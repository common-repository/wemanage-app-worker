...
if (!class_exists('Nouvello_WeManage_Worker_OAuth')) :
    ...
    final class Nouvello_WeManage_Worker_OAuth
    {
        ...
        private function perform_basic_authentication()
        {
            ...
            if (!empty($get_array['consumer_key']) && !empty($get_array['consumer_secret'])) {
                $consumer_key    = sanitize_text_field(wp_unslash($get_array['consumer_key']));
                $consumer_secret = sanitize_text_field(wp_unslash($get_array['consumer_secret']));
            }
            ...
            if (!hash_equals($this->user->consumer_secret, $consumer_secret)) { // @codingStandardsIgnoreLine
                $this->set_error(new WP_Error('woocommerce_rest_authentication_error', __('Consumer secret is invalid.', 'nouvello-wp-management'), array('status' => 401)));
        ...
        public function perform_oauth_authentication()
        {
            ...
            if (empty($this->user)) {
                $this->set_error(new WP_Error('nouvello_rest_authentication_error', __('Consumer key is invalid.', 'nouvello-wp-management'), array('status' => 401)));
                return false;
        ...
        public function get_oauth_parameters()
        {
            ...
            if (!empty($errors)) {
                $message = sprintf(
                    ...
                    _n('Missing OAuth parameter %s', 'Missing OAuth parameters %s', count($errors), 'nouvello-wp-management'),
                    implode(', ', $errors)
                );
        ...
        private function check_oauth_signature($user, $params)
        {
            ...
            if (!uksort($params, 'strcmp')) {
                return new WP_Error('nouvello_rest_authentication_error', __('Invalid signature - failed to sort parameters.', 'nouvello-wp-management'), array('status' => 401));
            }
            ...
            if ('HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method']) {
                return new WP_Error('nouvello_rest_authentication_error', __('Invalid signature - signature method is invalid.', 'nouvello-wp-management'), array('status' => 401));
            }
            ...
            if (!hash_equals($signature, $consumer_signature)) { // @codingStandardsIgnoreLine
                return new WP_Error('nouvello_rest_authentication_error', __('Invalid signature - provided signature does not match.', 'nouvello-wp-management'), array('status' => 401));
            }
        ...
        private function check_oauth_timestamp_and_nonce($user, $timestamp, $nonce)
        {
            ...
            if (($timestamp < time() - $valid_window) || ($timestamp > time() + $valid_window)) {
                return new WP_Error('nouvello_rest_authentication_error', __('Invalid timestamp.', 'nouvello-wp-management'), array('status' => 401));
            }
            ...
            if (in_array($nonce, $used_nonces, true)) {
                return new WP_Error('nouvello_rest_authentication_error', __('Invalid nonce - nonce has already been used.', 'nouvello-wp-management'), array('status' => 401));
            }
